...
| 模块 | 功能 | Demo 效果 |
|---|---|---|
| 登录与角色分流 | 账号 `111/111` 进入用户端，`222/222` 进入管理端 | 登录页 + 角色路由 |
| 管理员后台 | 查看知识库文件、上传 PDF/DOC、触发知识库重建 | 文件列表 + 上传区 + 重建进度 |
...
### Phase 5：登录、管理与 polish
- [x] 5.1 新增登录页与角色分流（普通用户 / 系统管理员）
- [x] 5.2 新增管理员后台：文件列表、PDF/DOC 上传、知识库重建
- [x] 5.3 添加加载动画、空状态、错误提示
- [x] 5.4 移动端响应式适配（基础适配已完成）
- [ ] 5.5 录制 Demo 演示视频 / 准备答辩 PPT
...
| 账号 | 密码 | 角色 |
|---|---|---|
| `111` | `111` | 普通用户（进入原有业务界面） |
| `222` | `222` | 系统管理员（进入后台管理界面） |
...
1. 使用账号 `222` / 密码 `222` 登录
2. 在管理员后台上传新的 PDF / DOC / DOCX / TXT / MD 文件
3. 点击"开始重建"，等待进度完成
4. 重建成功后，新内容立即可被智能问答检索
